...
const actions = {
  ...
  postRequestHandler({ dispatch,commit, state }, dataInfo) {
    ...
    if (!!dataInfo.apiBasic.apiCont.reqHeaders && dataInfo.apiBasic.apiCont.reqHeaders.length > 0) {
    ...
    Axios[methods](`/${appId}${url}`,param,{
      ...
      if (!!err.message.split('code')[1] && err.message.split('code')[1].trim() !== '200'
      ) {
...
const mutations = {
  ...
  changeCurrentApiData(state, dataList) {
    ...
    if(!['card','loop','flex','tabs'].includes(dataList.col.type)){
      Vue.set(ssrWindow.appVue.$root.findComponentsByRef(dataList.col.ref).$parent.record.col, 'apiData', dataList.resdata);
      const colType = dataList.col.type;
      if(colType.includes('tableCount') || colType.includes('Table')){
        Vue.set(ssrWindow.appVue.$root.findComponentsByRef(dataList.col.ref).$parent.record.col.tableHeaderList, 'value',  dataList.resdata.columns);
      }
    }else if(!!ssrWindow.appVue.$root.findComponentsByRef(dataList.col.key).record && ssrWindow.appVue.$root.findComponentsByRef(dataList.col.key).record.indraggable) {
      Vue.set(ssrWindow.appVue.$root.findComponentsByRef(dataList.col.key).record.col, 'apiData', dataList.resdata);
    }
...
function callbackCode(code,that,resData){ //接口返回code
...
//处理api参数逻辑
function getResParams(info,that,methods,apiData){ //解析参数
  let _this = that;
  let ResParams = {};
  let apiBasic = info.apiBasic;
...
function commonGetReqParamsValue(originType,modelValue,_this,type){
  let value;
  if(originType == 1){
    value = (!ssrWindow.appVue.$root.findComponentsByRef(modelValue)) ? '' : ssrWindow.appVue.$root.findComponentsByRef(modelValue).getValue();
...
function setGlobalVar(api,data){
  ...
  if(api.apiCont.globalVariableBasic.length == 1 && !api.apiCont.globalVariableBasic[0].key) return;
